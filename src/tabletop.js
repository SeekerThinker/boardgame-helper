...
import {
  ...
  addTracker, removeTracker, trackerEntityIds, trackerValue, adjustTracker, setTrackerValue,
...
const I18N = {
  zh: {
    launcher: '桌面 OS', title: '高级桌游助手', subtitle: '把桌上的记账、阶段、状态和主持负担交给一个可配置工作台。',
    ...
    activeTemplate: '当前模板', coverage: '工作台能力', coverageText: '通用计数 · 阶段流程 · 团队 · 私密身份 · 公式计分 · 战役检查点',
    emptyParticipants: '还没有参与者。可以同步主应用玩家，或单独添加。', savedLocal: '所有高级助手数据只保存在本机。',
    addTracker: '添加追踪器', trackerName: '名称', scope: '范围', global: '公共', participant: '每位玩家', team: '每个团队', step: '步长', min: '最小', max: '最大',
    noTrackers: '当前没有追踪器。', addPhase: '添加阶段', cycle: '循环', previous: '上一步', next: '下一步', noPhases: '当前没有阶段。', phaseNote: '备注',
    addTeam: '添加团队', noTeams: '当前没有团队。', members: '成员', role: '身份', faction: '阵营', secret: '私密', note: '主持备注', reveal: '私密查看', hide: '隐藏身份', clear: '清除',
    ...
    confirmTemplate: '应用模板会重置追踪器、阶段、团队、身份和高级计分表，但保留参与者。继续吗？', confirmReset: '开始新场景会清空追踪值、身份和高级计分值，并推进战役局数。继续吗？',
    ...
    revealFor: '仅给这位玩家看', passDevice: '请把设备交给对应玩家后再查看。', noRole: '尚未设置身份',
    templateHint: '模板只是一组可编辑的工作流起点，不替代官方规则。'
  },
  en: {
    launcher: 'Table OS', title: 'Advanced Table Assistant', subtitle: 'A configurable workspace for table state, phases, teams, roles, scoring and campaigns.',
    ...
    activeTemplate: 'Active template', coverage: 'Workspace capabilities', coverageText: 'Counters · phase flow · teams · private roles · formula scoring · campaign checkpoints',
    emptyParticipants: 'No participants yet. Sync the main game roster or add assistant-only participants.', savedLocal: 'All advanced-assistant data stays on this device.',
    ...
    noTrackers: 'No trackers yet.', addPhase: 'Add phase', cycle: 'Cycle', previous: 'Previous', next: 'Next', noPhases: 'No phases yet.', phaseNote: 'Note',
    addTeam: 'Add team', noTeams: 'No teams yet.', members: 'Members', role: 'Role', faction: 'Faction', secret: 'Private', note: 'Moderator note', reveal: 'Private reveal', hide: 'Hide role', clear: 'Clear',
    ...
    confirmTemplate: 'Applying a template resets trackers, phases, teams, roles and the advanced score sheet while preserving participants. Continue?', confirmReset: 'Starting a new scenario clears tracker values, roles and advanced scores, and advances the campaign session. Continue?',
    ...
    revealFor: 'For this player only', passDevice: 'Pass the device to the matching player before revealing.', noRole: 'No role assigned',
    templateHint: 'Templates are editable workflow starters, not replacements for official rules.'
  }
};
...
function readGamePlayers() {
  for (const key of [STORAGE_KEY, LEGACY_GAME_STATE_KEY]) {
    try {
      ...
      if (Array.isArray(parsed?.players) && parsed.players.length) return parsed.players;
    } catch (_) {}
  }
  return [];
}
...
function renderLauncher() {
  ...
  launcher.dataset.active = state.trackers.length || state.phases.items.length || state.teams.length ? 'true' : 'false';
}
...
function renderShell() {
  return `
    <div class="tableos-backdrop" data-os-action="close">
      <section class="tableos-sheet" role="dialog" aria-modal="true" aria-labelledby="tableos-title">
        <header class="tableos-header">
          ...
          <button class="tableos-icon" type="button" data-os-action="close" aria-label="${attr(tr('close'))}">×</button>
        </header>
        <div class="tableos-toolbar">
        ...
        <nav class="tableos-nav" aria-label="${attr(tr('title'))}">
          ${SECTION_IDS.map(id => `<button type="button" data-os-section="${id}" class="${state.ui.activeSection === id ? 'active' : ''}" aria-current="${state.ui.activeSection === id ? 'page' : 'false'}">${esc(tr(id))}</button>`).join('')}
        </nav>
        <div class="tableos-content">${renderSection()}</div>
        <footer class="tableos-footer"><span>${esc(tr('savedLocal'))}</span><button type="button" class="tableos-btn danger" data-os-action="reset">${esc(tr('reset'))}</button></footer>
...
function renderOverview() {
  ...
  return `
    ...
    <section class="tableos-card">
      <div class="tableos-card-head"><div><span>${esc(tr('participants'))}</span><h3>${state.participants.length} / ${MAX_TABLE_OS_PARTICIPANTS}</h3></div><button class="tableos-btn" type="button" data-os-action="add-participant">${esc(tr('addParticipant'))}</button></div>
      ${state.participants.length ? `<div class="tableos-participants">${state.participants.map(participant => `
        <div class="tableos-person" style="--person:${participant.color}">
          <span class="tableos-dot"></span><input value="${attr(participant.name)}" data-os-participant-name="${participant.id}" maxlength="32">
          <small>${esc(participant.sourcePlayerId ? tr('sourceGame') : tr('localOnly'))}</small>
          <button class="tableos-mini danger" type="button" data-os-remove-participant="${participant.id}" aria-label="${attr(tr('remove'))}">×</button>
        </div>`).join('')}</div>` : `<p class="tableos-empty">${esc(tr('emptyParticipants'))}</p>`}
    </section>`;
}
...
function renderTrackers() {
  return `
    <section class="tableos-card">
      <div class="tableos-card-head"><div><span>${esc(tr('trackers'))}</span><h3>${state.trackers.length} / ${MAX_TRACKERS}</h3></div><button class="tableos-btn primary" type="button" data-os-action="add-tracker">${esc(tr('addTracker'))}</button></div>
...
function renderTracker(tracker) {
  ...
  return `<article class="tableos-module">
    <div class="tableos-module-head">
      <input class="tableos-title-input" value="${attr(tracker.name)}" data-os-tracker-name="${tracker.id}" maxlength="32">
      <select data-os-tracker-scope="${tracker.id}">
        <option value="global" ${tracker.scope === 'global' ? 'selected' : ''}>${esc(tr('global'))}</option>
        <option value="participant" ${tracker.scope === 'participant' ? 'selected' : ''}>${esc(tr('participant'))}</option>
        <option value="team" ${tracker.scope === 'team' ? 'selected' : ''}>${esc(tr('team'))}</option>
      </select>
...
function renderPhases() {
  ...
  return `
    <section class="tableos-card">
      <div class="tableos-card-head"><div><span>${esc(tr('phases'))}</span><h3>${active ? esc(active.name) : '—'} · ${esc(tr('cycle'))} ${state.phases.cycle}</h3></div><button class="tableos-btn primary" type="button" data-os-action="add-phase">${esc(tr('addPhase'))}</button></div>
      ${state.phases.items.length ? `<div class="tableos-phase-controls"><button class="tableos-btn" type="button" data-os-action="prev-phase">← ${esc(tr('previous'))}</button><button class="tableos-btn primary" type="button" data-os-action="next-phase">${esc(tr('next'))} →</button></div>` : ''}
      <div class="tableos-phase-list">${state.phases.items.map((phase, index) => `<article class="tableos-phase ${index === state.phases.activeIndex ? 'active' : ''}">
        <button class="tableos-phase-index" type="button" data-os-phase-active="${index}">${index + 1}</button>
        <div><input value="${attr(phase.name)}" data-os-phase-name="${phase.id}" maxlength="32"><input value="${attr(phase.note)}" data-os-phase-note="${phase.id}" maxlength="120" placeholder="${attr(tr('phaseNote'))}"></div>
        <button class="tableos-mini danger" type="button" data-os-remove-phase="${phase.id}">×</button>
      </article>`).join('') || `<p class="tableos-empty">${esc(tr('noPhases'))}</p>`}</div>
    </section>`;
}

function renderTeamsRoles() {
  return `
    <div class="tableos-grid two align-start">
      <section class="tableos-card">
        <div class="tableos-card-head"><div><span>${esc(tr('teams'))}</span><h3>${state.teams.length} / ${MAX_TEAMS}</h3></div><button class="tableos-btn primary" type="button" data-os-action="add-team">${esc(tr('addTeam'))}</button></div>
...
function renderCampaign() {
  return `
    <section class="tableos-card">
      <div class="tableos-card-head"><div><span>${esc(tr('campaign'))}</span><h3>${state.campaign.enabled ? esc(state.campaign.name || tr('campaign')) : 'OFF'}</h3></div><label class="tableos-switch"><input type="checkbox" data-os-campaign-enabled ${state.campaign.enabled ? 'checked' : ''}><span>${esc(tr('campaignEnabled'))}</span></label></div>
      <div class="tableos-form-grid campaign-form">
        <label>${esc(tr('campaignName'))}<input value="${attr(state.campaign.name)}" data-os-campaign="name" maxlength="60"></label>
        <label>${esc(tr('chapter'))}<input value="${attr(state.campaign.chapter)}" data-os-campaign="chapter" maxlength="60"></label>
        <label>${esc(tr('sessionNumber'))}<input type="number" min="1" max="9999" value="${state.campaign.sessionNumber}" data-os-campaign="sessionNumber"></label>
        <label class="wide">${esc(tr('notes'))}<textarea rows="6" maxlength="4000" data-os-campaign="notes">${esc(state.campaign.notes)}</textarea></label>
      </div>
      <div class="tableos-card-head sub"><div><span>${esc(tr('flags'))}</span><h3>${state.campaign.flags.filter(flag => flag.checked).length} / ${state.campaign.flags.length}</h3></div><button class="tableos-btn" type="button" data-os-action="add-flag">${esc(tr('addFlag'))}</button></div>
      <div class="tableos-flag-list">${state.campaign.flags.map(flag => `<div><label><input type="checkbox" data-os-flag-toggle="${flag.id}" ${flag.checked ? 'checked' : ''}><input value="${attr(flag.name)}" data-os-flag-name="${flag.id}" maxlength="50"></label><button class="tableos-mini danger" type="button" data-os-flag-remove="${flag.id}">×</button></div>`).join('') || `<p class="tableos-empty">${esc(tr('noFlags'))}</p>`}</div>
    </section>`;
}
...
function renderRoleReveal(participantId) {
  ...
  return `<div class="tableos-secret" role="dialog" aria-modal="true"><div><p>${esc(tr('revealFor'))}</p><h3>${esc(participant.name)}</h3><p class="tableos-secret-warning">${esc(tr('passDevice'))}</p><div class="tableos-secret-role"><strong>${esc(role?.role || tr('noRole'))}</strong>${role?.faction ? `<span>${esc(role.faction)}</span>` : ''}${role?.note ? `<p>${esc(role.note)}</p>` : ''}</div><button class="tableos-btn primary wide" type="button" data-os-action="hide-role">${esc(tr('hide'))}</button></div></div>`;
}
...
function bindEvents() {
  document.addEventListener('click', event => {
    const target = event.target instanceof Element ? event.target.closest('[data-os-action],[data-os-section],[data-os-remove-participant],[data-os-tracker-delta],[data-os-remove-tracker],[data-os-phase-active],[data-os-remove-phase],[data-os-remove-team],[data-os-role-reveal],[data-os-role-clear],[data-os-remove-score],[data-os-flag-toggle],[data-os-flag-remove]') : null;
    if (!target) return;
    const d = target.dataset;
    if (d.osAction === 'close') {
      if (event.target.closest('.tableos-sheet') && !event.target.closest('[data-os-action="close"]')) return;
      isOpen = false; revealedParticipantId = null; render(); return;
    }
    if (d.osAction === 'hide-role') { revealedParticipantId = null; render(); return; }
    ...
    if (d.osAction === 'apply-template') {
      if (!confirm(tr('confirmTemplate'))) return;
      applyAssistantTemplate(state, selectedTemplateId, { preserveParticipants: true }); persist(); render(); return;
    }
    ...
    if (d.osRoleReveal) { revealedParticipantId = d.osRoleReveal; render(); return; }
  ...
  document.addEventListener('keydown', event => {
    if (event.key === 'Escape' && revealedParticipantId) { revealedParticipantId = null; render(); return; }
