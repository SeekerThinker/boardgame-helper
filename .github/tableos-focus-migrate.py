...
anchor = """function restoreRenderContext(context) {
...
function render() {"""
focus_helpers = """function restoreRenderContext(context) {
  const sheet = document.querySelector('.tableos-sheet');
  if (sheet) sheet.scrollTop = context.scrollTop;
  if (!context.focusSelector) return;
  const next = document.querySelector(context.focusSelector);
...
function render() {"""
source = replace_once(source, anchor, focus_helpers, 'focus helpers')
source = replace_once(
    source,
...
source = replace_once(
    ...
    'keydown focus handling',
)
path.write_text(source)

test_path = Path('tests/e2e/run-table-os-e2e.js')
test = test_path.read_text()
helper_anchor = """async function assertNoHorizontalOverflow(page, label) {
...
"""
test = replace_once(test, helper_anchor, helper, 'focus test helper')
test = replace_once(
    test,
...
""",
    'nested dialog focus return test',
)
test = replace_once(
    test,
...
""",
  ...
  await page.getByRole('button', { name: '界面语言' }).click();
""",
    'launcher focus return test',
)
test = replace_once(
    test,
    "'unary formula operators',",
    "'unary formula operators', 'modal focus trap', 'nested reveal focus return', 'launcher focus return',",
    'summary',
)
