...
async function runPrimaryFlow() {
  ...
  const threatValue = threat.locator('[data-os-tracker-value]').first();
  const plus = threat.locator('[data-os-tracker-delta]').filter({ hasText: '+' }).first();
  ...
  // Phase changes are reversible even though the original control re-renders the list.
  ...
  await page.locator('[data-tableos-companion-action="undo"]').click();
  assert.equal(await page.locator('.tableos-phase.active strong').textContent(), activeBefore, 'phase change can be undone');
...
async function run() {
  ...
  console.log(JSON.stringify({
    ...
    status: 'PASS',
    checks: [
      'live main-game context', 'timer continuity', 'tracker undo', 'direct-value undo', 'phase undo',
      'main-timer bridge', 'contextual accessibility labels', '44px live touch targets', '320px overflow'
    ]
  }, null, 2));
}
